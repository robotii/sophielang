...
#
# For Sophie benchmarks, it can also compare against a "baseline". That's a
# recorded result of a previous run of the Sophie benchmarks. This is useful --
# critical, actually -- for seeing how Sophie performance changes. Generating a
...
SOPHIE_DIR = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
SOPHIE_BIN = os.path.join(SOPHIE_DIR, 'Sophie','bin','Release')
BENCHMARK_DIR = os.path.join(SOPHIE_DIR, 'test', 'benchmark')
...
LANGUAGES = [
  ("sophie",           [os.path.join(SOPHIE_BIN, 'sophie')], ".sophie"),
  ("lua",            ["lua"],                          ".lua"),
...
def run_benchmark_language(benchmark, language, benchmark_result):
  ...
  if language[0] == "sophie":
    ...
    # Hack: assumes sophie gets run first.
    sophie_score = benchmark_result["sophie"]["score"]
    ratio = 100.0 * sophie_score / score
